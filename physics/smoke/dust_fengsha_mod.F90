    ...
    do j=jts,jte
       do i=its,ite
          ...
          if(xland(i,j).lt.1.5)then
             ...
             gravsm=100.*smois(i,1,j)/((1.-maxsmc(isltyp(i,j)))*(2.65*(1.-clay(i,j))+2.50*clay(i,j)))
...
!                  bems, g, drylimit, dust_alpha, dust_gamma, R, uthr(i,j))
 ...
 !      gamma, R, uthres)
  ...
  subroutine source_dust(imx, jmx, lmx, nmx, smx, dt1, tc, ustar, massfrac, &
    ...
    ! *         RHOSOIL   Density of Soil                               (kg/m3)
    ! *         SALT      total saltation                                  
    ! *         SALTBIN   total saltation in bin
    ! *
    ! *  Output:
    ...
    REAL(kind_phys) :: saltbin(smx)   
    REAL(kind_phys) :: den(smx) 
    REAL(kind_phys) :: diam(smx)
    REAL(kind_phys) :: dvol(nmx)
    REAL(kind_phys) :: distr_dust(nmx)
    REAL(kind_phys) :: dlndp(nmx)
    REAL(kind_phys) :: dsurface(smx)
    REAL(kind_phys) :: ds_rel(smx)
    REAL(kind_phys) :: u_ts0
    REAL(kind_phys) :: u_ts
    REAL(kind_phys) :: dsrc
    REAL(kind_phys) :: dmass
    REAL(kind_phys) :: dvol_tot
    REAL(kind_phys) :: salt
    REAL(kind_phys) :: emit
    REAL(kind_phys) :: emit_vol
    REAL(kind_phys) :: stotal
    REAL(kind_phys) :: rhoa
    REAL(kind_phys) :: g
    INTEGER   :: i, j, n
